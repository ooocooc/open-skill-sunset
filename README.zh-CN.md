...
2026-09-01，我们用当前版本扫描了维护者实际使用的两个 Skill 目录，并将结果脱敏：

| 实际问题 | 扫描结果 | 它帮你避免什么 |
| --- | --- | --- |
| 在 Mac 上扫描到一个 Skill 中的 5 个 Windows 专用路径 | `UPDATE` | 提前发现当前系统不能直接执行的指令；先确认它是否本来就只给 Windows 使用 |
| 一个入口文件有 2,517 行，约 17,984 token | `DEMOTE` | 每次任务都携带大量低频说明 |
| 4 条规则要求每个任务都调用工具 | `TEST` | 未经对照测试就误删规则，或长期承受多余调用 |
| 一条规则写了具体模型名称 | `UPDATE` | 提醒人工复核；本次确认它仍是现行、特定用途说明，所以保留 |

原始路径、Skill 名称和指令正文没有公开。每个案例都提供了可以在仓库中运行的脱敏样本：[查看完整中文案例](docs/cases.zh-CN.md) · [运行公开样本](examples/cases/README.md)。
...
0.3.0 已实现静态检查、保守重复退役、双语报告、路径脱敏、带执行门槛的命令实验框架、CI 严重度退出码、对抗性输出测试和本地 Skill 启用检查清单；自动读取会话使用情况和任务质量的功能仍属于后续证据层。
